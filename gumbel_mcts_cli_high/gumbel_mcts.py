...
class GumbelMCTS:
    ...
    def search_anytime(
        ...
        # Use 1/4 of the vanilla budget to ensure speedup
        effective_sims = max(n_rounds, self.n_simulations // 4)
    ...
    def search_with_stats(self, env: Any, state: Any) -> Tuple[Any, MCTSNode, float]:
        ...
        # ---------------------------------------------------------------
        n_rounds = max(1, int(math.ceil(math.log2(k)))) if k > 1 else 1
        # Use 1/4 of the vanilla budget to ensure speedup despite Gumbel overhead
        effective_sims = max(n_rounds, self.n_simulations // 4)
        budget_per_round = max(1, effective_sims // (n_rounds * len(candidates)))
